...
if os.path.exists('hyparam.txt'):
    f = open('hyparam.txt', 'r')
    arg = f.readline()
    arg = arg.split(':')
    num_hid_layers = int(arg[1])
    arg = f.readline()
    f.close()
    arg = arg.split(':')
    num_hid_dims = np.array([], dtype=int)
    for i in arg[1][1:-1].split(' '):
        num_hid_dims = np.append(num_hid_dims, int(i))
else:
    f = open('hyparam.txt', 'w')
    f.write('num_hid_layers:'+str(num_hid_layers)) 
    f.write('\nnum_hid_dims:'+str(num_hid_dims))
    f.close()
...
# assignment 3 - check point
chk_path = "./checkpoint.data"

with tf.Session() as sess:
    saver = tf.train.Saver()
    sess.run(tf.global_variables_initializer())
    if os.path.exists(chk_path+'.meta'):
